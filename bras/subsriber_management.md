...
найдено правило, соответствующее пакету, для его маршрутизации используется
...
поэтому для PBR правил необходимо использовать таблицы, хранящие IP адреса
...
vlan идентификаторов пакета и номера порта, на котором получен пакет.
...
## Таблицы идентификаторов заблокированных подписчиков
 ...
 * IP_TABLE_NAME - имя таблицы с IP адресами. Она используется для хранения id
заблокированных L2/L3 connected подписчиков, т.к. идентификатор таких подписчиков 
- это IP адрес подписчика.
	
 * L2_TABLE_NAME - имя таблицы c L2 идентификаторами подписчиков. В нее добавляются
L2 ID подписчиков, подключаемых по принципу vlan per subsriber.

!! Важно: таблицы маршрутизации, правила PBR, таблицы ID подписчиков,
